...
def cmd_train(train_start: str, train_end: str, test_start: str, test_end: str, train_csv: Optional[str] = None):
    ...
    1) read training csv
    2) generate features
    3) train, validate, save model into models.modeling
...
def cmd_simulate(price_plan_path: str, horizon: int, out_csv: Optional[str], constraints_path: Optional[str]):
    """
    1) read planned price over a horizon
    2) simulate sales performance (predict units, revenue, margin)
    3) Save results with the specified name out_svc in the same directory as pipeline.py
    """
